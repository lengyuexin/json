
## 自定义封装的数据接口

* [article.json --每日一句](https://lengyuexin.github.io/json/article.json)

* [category.json --图片分享](https://lengyuexin.github.io/json/category.json)

* [joke.json    ---笑话](https://lengyuexin.github.io/json/joke.json)

* [music.json  -网易云音乐接口](https://lengyuexin.github.io/json/music.json)

* [slide.json  --轮播图](https://lengyuexin.github.io/json/slide.json)

* [text.json  --毒鸡汤](https://lengyuexin.github.io/json/text.json)

* [goods.json ---商品数据列表，来自京东](https://lengyuexin.github.io/json/goods.json)

* [story.json -- 古风短篇](https://lengyuexin.github.io/json/story.json)

* [study.json--学习资源](https://lengyuexin.github.io/json/study.json)

* [app.json --- 饿了么app模拟数据](https://lengyuexin.github.io/json/app.json)

* [jianshu/search.json 简书搜索数据](https://lengyuexin.github.io/json/jianshu/search.json)

* [jianshu/list.json 简书文章列表](https://lengyuexin.github.io/json/jianshu/list.json)

* [jianshu/detail.json 简书文章详情](https://lengyuexin.github.io/json/jianshu/detail.json)

* [kugou数据接口](https://lengyuexin.github.io/json/kugou.json)
